    ...
    loop {
        ...
        match connect_gateway(&config) {
            Ok(mut socket) => {
                ...
                if let Err(error) = connected_loop(&mut socket, &router, &rx) {
                    router.gateway_disconnected(error);
                }
            }
            Err(error) => router.gateway_disconnected(error),
        }
    ...
    loop {
        ...
        match socket.read() {
            ...
            Ok(Message::Binary(data)) => {
                handle_event(socket, router, serde_json::from_slice(data.as_ref()))?;
            }
