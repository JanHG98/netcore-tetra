...
class SipSwitch:
    ...
    def normalize_tetra_number(self, number: str) -> tuple[int | None, str]:
        # PBXs also address radios as T<ISSI>. Only remove that explicit
        # marker; stripping arbitrary letters can silently call another SSI.
        cleaned = number.strip()
        if re.fullmatch(r"[Tt][0-9]+", cleaned):
            cleaned = cleaned[1:]
        if not re.fullmatch(r"[0-9]+", cleaned):
            return None, "destination must be numeric or T followed by digits"
    ...
    def _render_extensions(self) -> str:
        ...
        # Resolve the managed script independently of the distribution's
        # astagidir, including existing configs that still use the basename.
        if agi == "netcore-sip-route.py":
            agi = "/var/lib/asterisk/agi-bin/netcore-sip-route.py"
