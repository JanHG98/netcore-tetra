...
<script>
const el=id=>document.getElementById(id);let assets=[],jobs=[],libraryStatus=null,centralTtsStatus=null,centralTtsVoices=[],centralTtsTemplates=[],centralTtsTemplateId='';const pages=[...document.querySelectorAll('.page')];document.querySelectorAll('nav button').forEach(b=>b.onclick=()=>{document.querySelectorAll('nav button').forEach(x=>x.classList.remove('active'));b.classList.add('active');pages.forEach(p=>p.classList.toggle('active',p.id===b.dataset.page));if(b.dataset.page==='dispatch')showPreview();if(b.dataset.page==='tts')loadCentralTts()});
...
function renderSelects(){const shadow=libraryStatus?.operating_mode==='shadow';const approved=assets.filter(a=>a.state==='ready'&&a.approval==='approved');const selectable=shadow?approved:approved.filter(a=>a.broadcast_ready);const currentDispatch=el('dAsset').value;const options=selectable.map(a=>`<option value="${a.asset_id}">${esc(a.title)} (${a.kind}${shadow&&!a.broadcast_ready?' · Shadow-Metadatum':''})</option>`).join('');el('dAsset').innerHTML=options||`<option value="">${shadow?'Keine freigegebenen, bereiten Assets':'Keine TETRA-sendefähigen Assets'}</option>`;if(selectable.some(a=>a.asset_id===currentDispatch))el('dAsset').value=currentDispatch;el('dAsset').disabled=selectable.length===0;const hint=el('dispatchAssetHint');if(hint){if(shadow){hint.innerHTML='<b>Shadow-Modus:</b> Freigegebene, bereite Assets können als Metadaten-Job eingereiht werden. Es werden keine Audioframes ausgesendet; ein TETRA-Cache ist dafür nicht erforderlich.'}else if(selectable.length===0&&approved.length>0){hint.innerHTML='<b>Keine TETRA-sendefähigen Assets:</b> '+approved.length+' freigegebene Assets besitzen nur eine Vorschau, aber keinen gepackten TETRA-Cache. Encoder konfigurieren oder eine .tacelp-Datei importieren.'}else{hint.innerHTML='<b>Authoritativer Modus:</b> Nur freigegebene Assets mit validiertem TETRA-Cache werden angeboten.'}}const previews=assets.filter(a=>a.preview_ready);const popts=previews.map(a=>`<option value="${a.asset_id}">${esc(a.title)}</option>`).join('');const current=el('previewAsset').value;el('previewAsset').innerHTML='<option value="">Vorschau wählen</option>'+popts;if(previews.some(a=>a.asset_id===current))el('previewAsset').value=current}
