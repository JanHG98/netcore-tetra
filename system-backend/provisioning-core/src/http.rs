...
<style>
:root{color-scheme:dark;--bg:#08111f;--panel:#101d30;--panel2:#152641;--line:#2b4264;--text:#eef5ff;--muted:#9fb1c9;--ok:#4ade80;--bad:#fb7185;--accent:#60a5fa;--warn:#fbbf24}*{box-sizing:border-box}body{margin:0;background:linear-gradient(135deg,#07101d,#0d1830);color:var(--text);font:14px system-ui,sans-serif}header{padding:22px 28px;border-bottom:1px solid var(--line);background:#0b1628e8;position:sticky;top:0;z-index:4}h1{margin:0;font-size:25px}header p{margin:5px 0 0;color:var(--muted)}main{padding:22px;max-width:1600px;margin:auto}.cards{display:grid;grid-template-columns:repeat(5,minmax(150px,1fr));gap:12px}.card,.panel{background:var(--panel);border:1px solid var(--line);border-radius:13px;padding:15px}.card b{font-size:23px;display:block}.card span{color:var(--muted)}.toolbar{display:flex;gap:8px;flex-wrap:wrap;margin:16px 0}.tabs button,.toolbar button,button{border:1px solid var(--line);background:var(--panel2);color:var(--text);padding:9px 13px;border-radius:8px;cursor:pointer}.tabs button.active,button.primary{background:#1d4ed8;border-color:#3b82f6}.danger{background:#651d32!important}.warn{background:#614611!important}.tab{display:none}.tab.active{display:block}table{border-collapse:collapse;width:100%;min-width:760px}th,td{border-bottom:1px solid var(--line);padding:9px;text-align:left}th{color:#bad0eb;position:sticky;top:85px;background:var(--panel)}.scroll{overflow:auto;max-height:70vh}.pill{padding:3px 7px;border-radius:999px;background:#263b58}.ok{color:var(--ok)}.bad{color:var(--bad)}.muted{color:var(--muted)}input,textarea,select{width:100%;background:#091526;color:var(--text);border:1px solid var(--line);border-radius:7px;padding:8px}label{display:block;margin:9px 0}.checks{display:grid;grid-template-columns:repeat(3,1fr);gap:6px}.checks label{display:flex;gap:6px;align-items:center}.checks input{width:auto}dialog{background:var(--panel);color:var(--text);border:1px solid var(--line);border-radius:14px;width:min(720px,95vw)}dialog::backdrop{background:#000a}.actions{display:flex;gap:7px;flex-wrap:wrap}.matrix th:first-child,.matrix td:first-child{position:sticky;left:0;background:var(--panel);z-index:2}.matrix td{text-align:center}.matrix input{width:20px;height:20px}.search{max-width:360px}@media(max-width:800px){.cards{grid-template-columns:1fr 1fr}.checks{grid-template-columns:1fr}header{position:static}th{top:0}}
</style></head><body>
...
<section id="devices" class="tab active panel"><div class="toolbar"><button class="primary" onclick="editDevice()">Gerät anlegen</button><input id="deviceSearch" class="search" placeholder="ISSI, Name oder Organisation filtern" oninput="renderDevices()"></div><div class="scroll"><table><thead><tr><th>ISSI</th><th>Name</th><th>Organisation</th><th>Gerät</th><th>Freigabe</th><th>Funkstatus</th><th>Aktionen</th></tr></thead><tbody id="deviceRows"></tbody></table></div></section>
<section id="groups" class="tab panel"><div class="toolbar"><button class="primary" onclick="editGroup()">Gruppe anlegen</button><input id="groupSearch" class="search" placeholder="GSSI oder Name filtern" oninput="renderGroups()"></div><div class="scroll"><table><thead><tr><th>GSSI</th><th>Name</th><th>Attach</th><th>Ruf</th><th>SDS</th><th>Notruf</th><th>Aktionen</th></tr></thead><tbody id="groupRows"></tbody></table></div></section>
<section id="matrix" class="tab panel"><p class="muted">Zeile = Gerät, Spalte = Gruppe. Haken setzen oder entfernen; Änderungen werden sofort an Group Core und anschließend an die TBS verteilt.</p><div class="toolbar"><input id="matrixSearch" class="search" placeholder="Geräte filtern" oninput="renderMatrix()"></div><div class="scroll"><table class="matrix"><thead id="matrixHead"></thead><tbody id="matrixBody"></tbody></table></div></section>
</main>
...
function renderDevices(){const q=$('deviceSearch').value.toLowerCase();$('deviceRows').innerHTML=state.subscribers.filter(s=>`${s.issi} ${s.display_name} ${s.organization}`.toLowerCase().includes(q)).map(s=>{const o=state.observed.find(x=>x.issi===s.issi),radio=o?.registered?`<span class="ok">eingebucht</span><br><small>${esc(o.serving_node||'')} ${o.last_rssi_dbfs==null?'':Number(o.last_rssi_dbfs).toFixed(1)+' dBFS'}</small>`:'<span class="muted">offline/unbekannt</span>';return `<tr><td><b>${s.issi}</b></td><td>${esc(s.display_name)}</td><td>${esc(s.organization)}</td><td>${esc(s.device_label)}</td><td>${s.enabled&&s.registration_allowed?'<span class="ok">freigegeben</span>':'<span class="bad">gesperrt</span>'}</td><td>${radio}</td><td class="actions"><button onclick="editDevice(${s.issi})">Bearbeiten</button><button class="warn" onclick="toggleBlock(${s.issi})">${s.enabled?'Sperren':'Freigeben'}</button><button class="danger" onclick="deleteDevice(${s.issi})">Löschen</button></td></tr>`}).join('')}
function renderGroups(){const q=$('groupSearch').value.toLowerCase();$('groupRows').innerHTML=state.groups.filter(g=>`${g.gssi} ${g.name}`.toLowerCase().includes(q)).map(g=>`<tr><td><b>${g.gssi}</b></td><td>${esc(g.name)}</td><td>${mark(g.attach_allowed)}</td><td>${mark(g.call_allowed)}</td><td>${mark(g.sds_allowed)}</td><td>${mark(g.emergency_allowed)}</td><td class="actions"><button onclick="editGroup(${g.gssi})">Bearbeiten</button><button class="danger" onclick="deleteGroup(${g.gssi})">Löschen</button></td></tr>`).join('')}function mark(v){return v?'<span class="ok">✓</span>':'<span class="bad">–</span>'}
function membership(issi,gssi){return state.memberships.find(m=>m.issi===issi&&m.gssi===gssi&&m.allowed)}
function renderMatrix(){const q=$('matrixSearch').value.toLowerCase(),groups=[...state.groups].sort((a,b)=>a.gssi-b.gssi),subs=[...state.subscribers].filter(s=>`${s.issi} ${s.display_name}`.toLowerCase().includes(q)).sort((a,b)=>a.issi-b.issi);$('matrixHead').innerHTML='<tr><th>Gerät</th>'+groups.map(g=>`<th title="${esc(g.name)}">${g.gssi}<br><small>${esc(g.name)}</small></th>`).join('')+'</tr>';$('matrixBody').innerHTML=subs.map(s=>`<tr><td><b>${s.issi}</b><br><small>${esc(s.display_name)}</small></td>${groups.map(g=>{const m=membership(s.issi,g.gssi);return `<td><input type="checkbox" ${m?'checked':''} onchange="setMembership(${s.issi},${g.gssi},this.checked)" title="${m?.locked?'gesperrte Mitgliedschaft':''}"><br><button type="button" onclick="editMembership(${s.issi},${g.gssi})" title="Details bearbeiten">⚙</button>${m?.auto_attach?'<br><small class="pill">auto</small>':''}${m?.locked?'<br><small class="pill">fix</small>':''}</td>`}).join('')}</tr>`).join('')}
