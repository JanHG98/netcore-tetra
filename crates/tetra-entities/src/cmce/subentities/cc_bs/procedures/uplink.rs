...
impl CcBsSubentity {
    ...
    pub(in crate::cmce::subentities::cc_bs) fn fsm_on_network_circuit_simplex_granted(
        ...
        let Some((call_id, call_snapshot)) = self.find_brew_individual_call(brew_uuid) else {
            tracing::debug!(
                "CMCE: network SIMPLEX_GRANTED for unknown uuid={} grant={} permission={}",
                brew_uuid,
                grant,
                permission
            );
        ...
        if !call_snapshot.is_active() || !call_snapshot.is_simplex() {
            tracing::trace!(
                "CMCE: ignoring {:?} SIMPLEX_GRANTED uuid={} call_id={} active={} simplex={}",
                call_snapshot.network_entity(),
                brew_uuid,
                call_id,
        ...
        if remote_grant != TransmissionGrant::Granted {
            tracing::trace!(
                "CMCE: ignoring {:?} SIMPLEX_GRANTED uuid={} call_id={} grant={:?}",
                call_snapshot.network_entity(),
                brew_uuid,
                call_id,
                remote_grant
        ...
        let Some((remote_party, local_party)) = Self::brew_individual_floor_parties(&call_snapshot) else {
            tracing::debug!(
                "CMCE: {:?} SIMPLEX_GRANTED uuid={} call_id={} without one network party",
                call_snapshot.network_entity(),
                brew_uuid,
                call_id
            );
        ...
        tracing::info!(
            "CMCE: {:?} SIMPLEX_GRANTED uuid={} call_id={} remote_issi={} local_issi={}",
            call_snapshot.network_entity(),
            brew_uuid,
            call_id,
    ...
    pub(in crate::cmce::subentities::cc_bs) fn fsm_on_network_circuit_simplex_idle(
        ...
        let Some((call_id, call_snapshot)) = self.find_brew_individual_call(brew_uuid) else {
            tracing::debug!(
                "CMCE: network SIMPLEX_IDLE for unknown uuid={} grant={} permission={}",
                brew_uuid,
                grant,
                permission
            );
        ...
        if !call_snapshot.is_active() || !call_snapshot.is_simplex() {
            tracing::trace!(
                "CMCE: ignoring {:?} SIMPLEX_IDLE uuid={} call_id={} active={} simplex={}",
                call_snapshot.network_entity(),
                brew_uuid,
                call_id,
                call_snapshot.is_active(),
        ...
        let Some((remote_party, local_party)) = Self::brew_individual_floor_parties(&call_snapshot) else {
            tracing::debug!(
                "CMCE: {:?} SIMPLEX_IDLE uuid={} call_id={} without one network party",
                call_snapshot.network_entity(),
                brew_uuid,
                call_id
            );
        ...
        if !call_snapshot.is_floor_held_by(remote_party.addr.ssi) {
            tracing::trace!(
                "CMCE: ignoring {:?} SIMPLEX_IDLE uuid={} call_id={} floor_holder={:?}",
                call_snapshot.network_entity(),
                brew_uuid,
                call_id,
        ...
        tracing::info!(
            "CMCE: {:?} SIMPLEX_IDLE uuid={} call_id={} remote_issi={} grant={} permission={}",
            call_snapshot.network_entity(),
            brew_uuid,
            call_id,
            remote_party.addr.ssi,
        ...
        if let Some(requester) = queued_request {
            let Some((requester_party, former_speaker_party)) = Self::individual_floor_parties(&call_snapshot, requester) else {
                tracing::warn!(
                    "CMCE: {:?} SIMPLEX_IDLE call_id={} had queued non-participant ISSI {}, dropping request",
                    call_snapshot.network_entity(),
                    call_id,
                    requester.ssi
                );
