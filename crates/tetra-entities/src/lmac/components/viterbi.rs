...
// Was: Vergibt für soft bit einen fachlich verständlichen Typnamen.
...
// Was: Vergibt für metric einen fachlich verständlichen Typnamen.
// Warum: Der Alias macht Signaturen lesbarer und hält technische Details aus dem aufrufenden Code heraus.
type Metric = i16;
...
// Was: Legt den festen Wert `K` für k fest.
// Warum: Der benannte Wert vermeidet schwer verständliche Zahlen oder Texte direkt in der Programmlogik und hält Änderungen zentral.
const K: usize = 5;

// Was: Legt den festen Wert `NUM_STATES` für num states fest.
// Warum: Der benannte Wert vermeidet schwer verständliche Zahlen oder Texte direkt in der Programmlogik und hält Änderungen zentral.
...
// Was: Vergibt für decision bitmap einen fachlich verständlichen Typnamen.
// Warum: Der Alias macht Signaturen lesbarer und hält technische Details aus dem aufrufenden Code heraus.
type DecisionBitmap = u16;

/// Number of states for a given constraint length.
// Was: Führt den Arbeitsschritt `num_states` für num states aus.
...
// Was: Bündelt die zusammengehörigen Werte für viterbi decoder in einem Datentyp.
...
// Was: Implementiert das zugehörige Verhalten für `ViterbiDecoder<N>`.
// Warum: Die Operationen bleiben dadurch direkt bei dem Datentyp, dessen Zustand sie lesen oder verändern.
impl<const N: usize> ViterbiDecoder<N> {
    // Was: Diese Funktion erstellt with polynomials.
...
// Was: Vergibt für TETRA viterbi decoder einen fachlich verständlichen Typnamen.
// Warum: Der Alias macht Signaturen lesbarer und hält technische Details aus dem aufrufenden Code heraus.
pub type TetraViterbiDecoder = ViterbiDecoder<4>;

// Was: Implementiert das zugehörige Verhalten für `TetraViterbiDecoder`.
// Warum: Die Operationen bleiben dadurch direkt bei dem Datentyp, dessen Zustand sie lesen oder verändern.
...
// Was: Vergibt für TETRA codec viterbi decoder einen fachlich verständlichen Typnamen.
// Warum: Der Alias macht Signaturen lesbarer und hält technische Details aus dem aufrufenden Code heraus.
pub type TetraCodecViterbiDecoder = ViterbiDecoder<3>;

// Was: Implementiert das zugehörige Verhalten für `TetraCodecViterbiDecoder`.
// Warum: Die Operationen bleiben dadurch direkt bei dem Datentyp, dessen Zustand sie lesen oder verändern.
...
// Was: Führt den Arbeitsschritt `dec_sb1` für dec sb1 aus.
// Warum: Der abgegrenzte Arbeitsschritt kann dadurch wiederverwendet, getestet und leichter verstanden werden.
pub fn dec_sb1(in_buf: &[u8], out_buf: &mut [u8], sym_count: usize) {
    // Was: Legt den festen Wert `MAX_SYM` für max sym fest.
    // Warum: Der benannte Wert vermeidet schwer verständliche Zahlen oder Texte direkt in der Programmlogik und hält Änderungen zentral.
...
mod tests {
    ...
    #[test]
    // Was: Prüft automatisch den Fall decoder.
